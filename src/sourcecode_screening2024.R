if (!("pacman" %in% rownames(installed.packages()))) {install.packages("pacman", lib = 'C:/R-library/R-4.4.0' )}
...
modeling_explanatory_q <- function(x, explanatory, link = "identity", autocor = autocor, tdist = tdist) {
  # family <- if(log == TRUE){gaussian(link="log")}else{gaussian()}
  formula <- variable ~ s(decimaldate, k = round(nrow(x)/2)) + s(decimal_during, bs = "cc", k = 13) + q15_log
  knots <- list(decimal_during = seq(0, 1, length=13))
  x <- drop_na(x, variable)
  if(autocor == TRUE)
  {out <- try(model(x, link, formula, knots))
...
modeling_expl <- function(x, link = "identity",expl_variable.type = "linear", autocor = autocor, tdist = tdist) {
  # family <- if(log == TRUE){gaussian(link="log")}else{gaussian()}
  if (expl_variable.type == "linear"){formula <- variable ~ s(decimaldate, k = round(nrow(x)/2)) + s(decimal_during, bs = "cc", k = 13) + exp_var
  }  else {
    formula <- variable ~ s(decimaldate, k = round(nrow(x)/2)) + s(decimal_during, bs = "cc", k = 13) + s(exp_var)
  }
  knots <- list(decimal_during = seq(0, 1, length=13))
  x <- drop_na(x, variable)
  if(autocor == TRUE)
...
screeningmodeling_exp <- function(.data,
                              datevar, #variabel med datum (i datumformat!)
                              values, # variabel med värden
                              expl_variable,
                              expl_variable.type = "linear",
                              link = "identity",
                              autocor = TRUE,
                              conf.type = "confidence",
                              conf.level=0.95, 
  ...
  .data %>%
    mutate(variable = !!variable,
           date = !!datevar,
           exp_var = !!exp_var) %>%
    select(date, variable,exp_var, !!!nestvars) %>%
    group_by(!!!nestvars, date) %>%
    summarise_all(mean) %>%
    ungroup() %>%
    drop_na(variable) %>%
    ...
    ungroup() %>%
    mutate(
      fit = future_map(data, possibly(~ modeling_expl(.x,
                                                 link = link,
                                                 expl_variable.type = expl_variable.type,
                                                 autocor = autocor, tdist = tdist),
                                      otherwise = NA_integer_,
                                      quiet = F),
                       .progress = T, seed=T),
...
add_individual_trend <- function(x,plot, y=NULL, title=NULL){
  if(nrow(x) != 1){stop("Filter out the variable (and/or station) you are interested in.")}
  annualterm <- predict(x$fit[[1]], newdata=x$data[[1]], type="terms")[,1]
  intercept <- x$fit[[1]]$coef["(Intercept)"]
  x$fderiv[[1]] %>%
    ...
    mutate(signif = !between(0, lower, upper),
           sign=sign(deriv),
           signif_sign = signif*sign*2) %>%
    ungroup %>% bind_cols(x$data[[1]],.) %>%
    mutate(trend = annualterm+intercept) %>%
    drop_na(variable) -> out
  
  plot + geom_line(aes(x = date, y=trend, color=as_factor(signif_sign), group=c(0)), lwd=1.5, data = out)+
    scale_color_manual(values=c("-1" = "#56B4E9",
                                "0" = "#F0E442",
                                "1" = "#D55E00",
                                "-2" = "#2c7bb6",
