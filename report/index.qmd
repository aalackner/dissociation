...
data_periods %>%
 mutate(weak_acids = (Sum.of.anions -(-org_charge + SO4_mol + `Cl (mmol/l)`/1000 + `NO3 (mol/l)` )) / Sum.of.anions) -> data_periods
...
data_with_weights %>% mutate(
  ...
  `Sum of Cations (meq/l)` = Sum.of.cations*1000
    ...
    SO4_mol, Alk_Acid_mol, org_charge, Sum.of.cations, sample_id, charge_diff
    )) %>% rename(
      `TOC (mg/l)` = TOC_mg_l,
      `Org. Charge Density (meq/g of C)` = org_charge_density_meq_g_C,
      `EC (mS/m)` = EC_mS_m,
      ...
      `Ionic strength` = Ionic.strength, 
      `Field pH` = pH_,
      `non-organic weak acids` = weak_acids 
    ) -> data_with_weights_1

variables = c('TOC (mg/l)', 'sVISa', 'Alk Acid (mmol/l)', 'Organic charge (meq/l)', 'Org. Charge Density (meq/g of C)', 'SO4 (mmol/l)', 'EC (mS/m)', 'Absorbance 420mm', 'Field pH',"Sum of Cations (meq/l)", "Al total (µmol/l)", "Si (mmol/l)", "Cl (mmol/l)" , "F (µmol/l)", "non-organic weak acids")
...
facet_labels <- c(
  ...
  `non-organic weak acids` = "f) NO weak acids (contribution)",
...
Acids in a water chemistry sample can be split into weak acids and strong acids. The weak acids can be further divided into organic acids and other weak acids. Here we look at how the contribution of these non-organic other weak acids has changed over time. 

The weak acid contribution is calculated as: 

$$
{\substack{\text{charge contribution of} \\ \text{ non-organic weak anions}}} =  \frac{\text{Sum of anions} - (\text{Organic anions} + \text{Sum of strong anions})}{\text{Sum of anions}}
$$
...
```{r}
data_periods %>%
 mutate(weak_acids = (Sum.of.anions -(-org_charge + SO4_mol + `Cl (mmol/l)`/1000 + `NO3 (mol/l)` )) / Sum.of.anions, sampling_date = as.Date(sampling_date)) -> data_periods 
```
