...
#|      - For the 126 stations charge density was modelled in this study.
...
#|    - 126 stations for which we could model charge density sucessfully for.
...
#| fig-alt: Climate change in the three periods.
#| layout-ncol: 2
discharge %>% mutate(q_spec= (median_q_m3_s / area_ARO_m2)*1000, p25_spec = (p25_q_m3_s / area_ARO_m2)*1000, p75_spec = (p75_q_m3_s / area_ARO_m2)*1000 ) %>% filter((year >= 1990 & year < 1998) ) %>%
group_by(mvm_id) %>% summarise(
...
discharge %>% mutate(q_spec= (median_q_m3_s / area_ARO_m2)*1000, p25_spec = (p25_q_m3_s / area_ARO_m2)*1000, p75_spec = (p75_q_m3_s / area_ARO_m2)*1000 ) %>% filter((year >= 1998 & year < 2012) ) %>%
group_by(mvm_id) %>% summarise(
...
discharge %>% mutate(q_spec= (median_q_m3_s / area_ARO_m2)*1000, p25_spec = (p25_q_m3_s / area_ARO_m2)*1000, p75_spec = (p75_q_m3_s / area_ARO_m2)*1000 ) %>% filter((year >= 2012 & year < 2024) ) %>%
group_by(mvm_id) %>% summarise(
...
In this section we test for normality of the data, compare the variation within stations and between stations and look at correlations betweeen the different water chemistry variables. For this we use the data of the 126 stations for which we sucessfully modelled organic charge. 
...
#|      - "Organic charge median and IQR distribution (in mol of charge per litre)"
#| label: fig-variation
#| layout-ncol: 2
...
plot_variable_distribution(data_1, "org_charge")
```
...
For each stations we split the data minto these three periods, then we test the data within each stations for changes between the periods using Kruskal-Wallis. 
...
change to meadn where normally distributed.
```{r}
#| cache.refresh: true
