...
data %>% mutate(year = year(sampling_date), TOC_mg_l = TOC_mol*12.01*1000, org_charge_density_meq_g_C = org_charge_eq_mg_C * 1000*1000,`Al3+ (µmol/l)` = Al.3 *1000000, `Si (mmol/l)` = Si_mol* 1000, `Cl (mmol/l)` = Cl_mol*1000, `Al total (µmol/l)` = Al_mol * 1000000, `F (µmol/l)` = Fluorid_mol *1000000, `NO3 (mol/l)` = NO3_N_mol) %>% select(year,EC_mS_m, sVISa,org_charge_density_meq_g_C,TOC_mg_l,SO4_mol,Alk_Acid_mol, sampling_date, Abs_F420_5cm, mvm_id, adom_doc, charge_diff, org_charge, Sum.of.cations, Sum.of.anions, pH_, Ionic.strength, sample_id, `Al3+ (µmol/l)`, `Si (mmol/l)`, `Cl (mmol/l)`,`Al total (µmol/l)`,`F (µmol/l)`, `NO3 (mol/l)`) -> data_1
```
...
sumtable(data_1_with_weights %>% mutate(
  ...
  `Sum of Cations (mmol/l)` = Sum.of.cations*1000
    ...
    SO4_mol, charge_diff, Alk_Acid_mol, org_charge, Sum.of.cations, Sum.of.anions, sample_id, mvm_id, weight
    )) %>% rename(
      `TOC (mg/l)` = TOC_mg_l,
...
data_periods <- data_1 %>% 
  mutate(
   ...
   weak_acids = 100*((Sum.of.anions -(-org_charge + 2*SO4_mol + `Cl (mmol/l)`/1000 + `NO3 (mol/l)` )) / Sum.of.anions), sampling_date = as.Date(sampling_date),
 strong_acids = 100*((2*SO4_mol + `Cl (mmol/l)`/1000 + `NO3 (mol/l)` )/Sum.of.anions),
...
#| fig-cap: Bargraphs of the contribution of non-organic weak acids (a), strong acids (b) and organic acids (c) for the three periods, for all stations weighted by the number of samples in each station. Calculated as the percentage of the total negative charge.
#| fig-height: 4
#| fig-width: 8

# Filter the data for the last three variables
last_three_variables <- c("NO weak acid contibution (%)", "Strong acid contibution (%)", "Organic acid contribution (%)")
...
facet_labels_last_three <- c(
  `NO weak acid contibution (%)`= "a) NO weak acid (%)",
  `Strong acid contibution (%)`= "b) Strong acids (%)",
  `Organic acid contribution (%)` = "c) Organic acids (%)"
)

# Create the plot with facets for the last three variables
ggplot(weighted_summary_last_three, aes(x = period, y = Median, fill = period)) +
  geom_bar(stat = "identity") +
  geom_errorbar(aes(ymin = Q25, ymax = Q75), width = 0.2) +
  labs(y = "Median Value", x = "Period") +
  theme_minimal() +
  theme(legend.position = "none",
   strip.text = element_text(hjust = 0)) +
  facet_wrap(~ variable, scales = "fixed", ncol = 3, labeller = labeller(variable = facet_labels_last_three) ) +
  scale_fill_brewer(palette = "Set2") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
percentage_changes <- summary_table %>%
  ...
  mutate(
    `1990-1997 - 1998-2012` = (`1998-2012` - `1990-1997`) / `1990-1997` * 100,
    `1990-1997 - 2012-2024` = (`2012-2024` - `1990-1997`) / `1990-1997` * 100,
    `1998-2012 - 2012-2024` = (`2012-2024` - `1998-2012`) / `1998-2012` * 100
  ) %>%
  select(mvm_id, `1990-1997 - 1998-2012`, `1990-1997 - 2012-2024`, `1998-2012 - 2012-2024`) %>%
...
percentage_changes <- summary_table %>%
  ...
  pivot_wider(names_from = period, values_from = median) %>%
  mutate(
    `1990-1997 - 1998-2012` = (`1998-2012` - `1990-1997`) / abs(`1990-1997`) * 100,
    `1990-1997 - 2012-2024` = (`2012-2024` - `1990-1997`) / abs(`1990-1997`) * 100,
    `1998-2012 - 2012-2024` = (`2012-2024` - `1998-2012`) / abs(`1998-2012`) * 100
  ) %>%
  select(mvm_id, `1990-1997 - 1998-2012`, `1990-1997 - 2012-2024`, `1998-2012 - 2012-2024`) %>%
  pivot_longer(cols = -mvm_id, names_to = "dunn_test.Comparison", values_to = "MedianChange")
...
library(tibble)
source("../src/sourcecode.R")
plot_loading_ggplot(opls_model = opls.model, response_label = "Org. charge density", vip.threshold = 1.0, text_size = 10)
