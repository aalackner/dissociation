...
```{r}
library(tidyverse)
setwd("/home/anna/repos/dissociation/report")
...
#| include: false
if (re.read == 're read'){
df_selected %>%  filter(!is.na(Abs_F420_5cm)) %>% # create sVISa and filter any variables where it does not exist
  group_by(mvm_id) %>%  # group by mvm_id to remocve andy station where we don't have enough data points
  filter(n() >= 50) %>%  
  ungroup() %>%
     select(mvm_id, Abs_F420_5cm, sampling_date,x, stationName) %>% mutate(SiteID = as.factor(mvm_id))%>% pivot_longer(cols=c("Abs_F420_5cm")) %>% 
    screeningmodeling(values=value,
                    datevar = sampling_date, 
                    link = "identity", 
                    conf.type = "conf",
                    ...
                    x,
                    stationName, name) ->
  trendplotdata_selected_Abs_F420_5cm
  trendplotdata_selected_Abs_F420_5cm %>% left_join(coordinates) %>% saveRDS(., file = "../data/trendplotdata_selected_Abs_F420_5cm.rds")
}  else if (re.read == 're load') {
  trendplotdata_selected_abs <- tryCatch({
    readRDS(file = "data/trendplotdata_selected_Abs_F420_5cm.rds")
  }, error = function(e) {
    message("An error occurred while reading the RDS file: ", e$message)
    NULL # Return NULL or an alternative value to handle the error gracefully
  })
...
```{r}
#| include: false
# Trend plot for Abs_F420_5cm

if (re.read != 're fig' & exists("trendplotdata_selected_Abs_F420_5cm")){
  fig <- trendplotdata_selected_Abs_F420_5cm %>%plot_proportions()+ ggtitle("Abs_420") + 
        theme(text=element_text(size=5), #change font size of all text
        axis.text=element_text(size=5), #change font size of axis text
        axis.title=element_text(size=6), #change font size of axis titles
        plot.title=element_text(size=6.5), #change font size of plot title
        legend.text=element_text(size=6), #change font size of legend text
        legend.title=element_text(size=5)) 
  fig %>% ggsave(file = "figures/Abs_F420_5cm_proportion_selected.png", width = 10,
  height = 6,
  units = "cm",
  dpi = 300) 
}  
...
```{r}
#| echo: false
#| fig-cap: "Proportion of poth sVISa and TOC"
#| fig-alt: All proportion of trends plot.
#| label: fig-all-prop
#| layout-ncol: 2

knitr::include_graphics(path = c("figures/TOC_proportion.png","figures/TOC_proportion_selected.png","figures/sVISa_proportion.png","figures/sVISa_proportion_selected.png" ), error = FALSE)
```
...
```{r}
#| echo: false
#| fig-cap: 136 stations with available data that were sucessfully modelled in this study. The size of the marker is related to the size of the catchment while the color shows the mean specific discharge (left), the annual precipitation (middle), and the annual mean tempertaur(right) of the catchment of the station.
...
print("Number of samples before")
...
## ADOM/DOC
```{r}
#| fig-cap: Histogram of ADOM/DOC for modelled samples for 173 stations (number of samples > 5).  
#| fig-alt: Histogram of ADOM/DOC.
#| label: fig-hist_adom_doc_after
#| fig-subcap:
#|    - All moddeled samples.
#|    - Samples accepted as sufficiently well modelled with a charge difference < 0.5%.
#| layout-ncol: 2
...
#| layout-ncol: 2
p1 <- ggplot(data %>% mutate(org_charge= org_charge_eq_mg_C * 1000 * 1000), aes(x = org_charge)) +
  geom_histogram(fill = "skyblue", color = "black", alpha = 0.7) +
  labs(title = "",
       x = "charge density (meq/g C)",
       y = "Frequency") +
  theme_minimal()

p1

 p1 %>% ggsave(file = "figures/charge_density_hist_bf.png", width = 8,
  height = 6,
  units = "cm",
  dpi = 300) 

p2 <- ggplot(data %>% filter(abs(charge_diff) < 0.5) %>% mutate(org_charge_density_meq_l = org_charge_eq_mg_C * 1000 * 1000), aes(x = org_charge_density_meq_l)) +
  geom_histogram(fill = "skyblue", color = "black", alpha = 0.7) +
  labs(title = "",
       x = "charge density (meq/g C)",
       y = "Frequency") +
...
p2 %>% ggsave(file = "figures/charge_density_hist_af.png", width = 8,
  height = 6,
  units = "cm",
  dpi = 300) 
...
```{r}
#| fig-cap: Scatterplot of charge density and adom/doc for samples before and after filtering. 

#| label: fig-scat_cd_adom_doc
#| fig-subcap:
#|    - Before filtering of charge difference < 0.5%
#|    - After filtering of charge difference < 0.5%
#| layout-ncol: 2
#| 
ggplot(data  %>% mutate(org_charge_density_meq_l = org_charge_eq_mg_C * 1000 * 1000), aes(x = org_charge_density_meq_l, y = adom_doc, fill = charge_diff)) +
...
ggplot(data %>% filter(abs(charge_diff) < 0.5) %>% mutate(org_charge_density_meq_l = org_charge_eq_mg_C * 1000 * 1000), aes(x = org_charge_density_meq_l, y = adom_doc, colour = log(Sum.of.cations))) +
  geom_point() +
  labs(title = "",
       x = "charge density (meq/g C)",
       y = "adom/doc") +
  ...
  ggplot(data %>% filter(abs(charge_diff) < 0.5) %>% mutate(org_charge_density_meq_l = org_charge_eq_mg_C * 1000 * 1000), aes(x = org_charge_density_meq_l, y = adom_doc)) +
  geom_point(color = "skyblue", color = "black", alpha = 0.7) +
  labs(title = "",
       x = "charge density (meq/g C)",
       y = "adom/doc") +
  theme_minimal()+
  geom_point(data = data %>% filter(abs(charge_diff) > 0.5) %>% mutate(org_charge_density_meq_l = org_charge_eq_mg_C * 1000 * 1000), aes(x = org_charge_density_meq_l, y = adom_doc), color = "black", color = "black", alpha = 0.7)
```
...
```{r}
#| eval: false
#| fig-cap: Change in discharge for the three periods 1990-1997, 1998-2011, 2012-2024. The median discharge (left), 25th percentage discharge (middle), 75th percentile discharge of each station is calculated and then plotted.
#|    - 25% discharge
#|    - 75% discharge
...
group_by(mvm_id) %>% summarise(
    log_mean_Q_spec = mean(log(q_spec)), log_p25_Q_spec = mean(log(p25_spec)), log_p75_Q_spec = mean(log(p75_spec)), 4logmean_Q = mean(log(median_q_m3_s))) %>% ungroup() %>% mutate(period = "2012-2024") -> discharge_p3
...
discharge_long <- discharge_all %>%
  pivot_longer(cols = c(mean_Q_spec ,p25_Q_spec, mean_Q),
               names_to = "variable", values_to = "value")
...
# `Al3+ (µmol/l)` = Al.3 *1000000,
data %>% filter(abs(charge_diff) < 0.5) %>% mutate(sVISa = Abs_F420_5cm/(TOC_mol*12.01*1000))  %>% mutate(year = year(sampling_date), TOC_mg_l = TOC_mol*12.01*1000, org_charge_density_meq_g_C = org_charge_eq_mg_C * 1000*1000, `Si (mmol/l)` = Si_mol* 1000, `Cl (mmol/l)` = Cl_mol*1000, `Al total (µmol/l)` = Al_mol * 1000000, `F (µmol/l)` = Fluorid_mol *1000000, `NO3 (mmol/l)` = NO3_N_mol * 1000) %>% select(year,EC_mS_m, sVISa,org_charge_density_meq_g_C,TOC_mg_l,SO4_mol,Alk_Acid_mol, sampling_date, Abs_F420_5cm, mvm_id, adom_doc, charge_diff, org_charge, Sum.of.cations, Sum.of.anions, pH_, Ionic.strength, sample_id,  `Si (mmol/l)`, `Cl (mmol/l)`,`Al total (µmol/l)`,`F (µmol/l)`, `NO3 (mmol/l)`) -> data_1

data %>% filter(abs(charge_diff) < 0.5) %>% mutate(sVISa = Abs_F420_5cm/(TOC_mol*12.01*1000))  %>% mutate(year = year(sampling_date), TOC_mg_l = TOC_mol*12.01*1000, org_charge_density_meq_g_C = org_charge_eq_mg_C * 1000*1000, `Si (mmol/l)` = Si_mol* 1000, `Cl (mmol/l)` = Cl_mol*1000, `Al total (µmol/l)` = Al_mol * 1000000, `F (µmol/l)` = Fluorid_mol *1000000, `NO3 (mmol/l)` = NO3_N_mol * 1000) %>% select(year,EC_mS_m, sVISa,org_charge_density_meq_g_C,TOC_mg_l,SO4_mol,Alk_Acid_mol, sampling_date, Abs_F420_5cm, mvm_id, adom_doc, charge_diff, org_charge, Sum.of.cations, Sum.of.anions, pH_, Ionic.strength, sample_id,  `Si (mmol/l)`, `Cl (mmol/l)`,`Al total (µmol/l)`,`F (µmol/l)`, `NO3 (mmol/l)`, Fluorid_mol,Na_mol,K_mol, Ca_mol,Fe_mol,SO4_mol,Si_mol, NH4_N_mol,Cl_mol, NO3_N_mol,Fluorid_mol,Na_mol,Mn_mol,Zn_mol,Cu_mol,Mg_mol) -> data_12


# saveRDS(data_12, file = "../results/r_py/data_1.rds")
```
...
data_table <- data_1_with_weights %>% mutate(
...
table_summary <- sumtable(data_table %>% select(-mvm_id), out = "return", group.weights = data_1_with_weights$weight,summ=c('notNA(x)',
...
table_summary %>% 
  mutate(MinValue = round(as.numeric(min_values[Variable]), digits = 3),
         MaxValue = round(as.numeric(max_values[Variable]), digits = 3)) %>% kable(., digits = 2)
```
...
#| layout-ncol: 2
plot_norm(data_1$org_charge_density_meq_g_C, name = "Organic charge density (meq/g of C)") -> p2

p2 %>% ggsave(file = "figures/charge_density_normal.png", width = 8,
  height = 6,
  units = "cm",
  dpi = 300) 
p2  
...
#| layout-ncol: 2
plot_norm(log(-data_1$org_charge_density_meq_g_C), name = "log (Organic charge density (meq/g of C))") -> p2

 ggsave(file = "figures/charge_density_normal_2.png", width = 8,
  height = 6,
  units = "cm",
  dpi = 300) 
p2  
...
plot_variable_distribution <- function(data, variable_name, title, legend = "Yes") {
...
weighted_median1 <- weightedMedian(data_1_with_weights$TOC_mg_l, w = data_1_with_weights$weight, na.rm = TRUE)
weighted_25th1 <- weightedQuantile(data_1_with_weights$TOC_mg_l, w = data_1_with_weights$weight, probs = 0.25, na.rm = TRUE)
weighted_75th1 <- weightedQuantile(data_1_with_weights$TOC_mg_l, w = data_1_with_weights$weight, probs = 0.75, na.rm = TRUE)
...
data_1 %>%
  ...
  geom_rect(aes(xmin = -Inf, xmax = Inf, ymin = weighted_25th1, ymax = weighted_75th1), fill = "lightblue", alpha = 0.2) +
  geom_hline(yintercept = weighted_median1, linetype = "dashed", color = "red") +
  geom_hline(yintercept = weighted_25th1, linetype = "dotted", color = "blue") +
  geom_hline(yintercept = weighted_75th1, linetype = "dotted", color = "blue")   + 
...
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"), axis.text.x=element_blank(), 
  ...
  labs(y = "TOC (mg/l)", x = "Station") + ylim(0,75)   -> p_TOC
...
# Calculate the weighted median, 25th percentile, and 75th percentile
weighted_median2 <- weightedMedian(data_1_with_weights$sVISa, w = data_1_with_weights$weight, na.rm = TRUE)
weighted_25th2 <- weightedQuantile(data_1_with_weights$sVISa, w = data_1_with_weights$weight, probs = 0.25, na.rm = TRUE)
weighted_75th2 <- weightedQuantile(data_1_with_weights$sVISa, w = data_1_with_weights$weight, probs = 0.75, na.rm = TRUE)

# Create the boxplot and add the horizontal lines for the weighted median, 25th, and 75th percentiles
data_1 %>%
  mutate(mvm_id = as.factor(mvm_id)) %>%
  ggplot(aes(x = mvm_id, y = sVISa)) +
  geom_rect(aes(xmin = -Inf, xmax = Inf, ymin = weighted_25th2, ymax = weighted_75th2), fill = "lightblue", alpha = 0.2) +
  geom_hline(yintercept = weighted_median2, linetype = "dashed", color = "red") +
  geom_hline(yintercept = weighted_25th2, linetype = "dotted", color = "blue") +
  geom_hline(yintercept = weighted_75th2, linetype = "dotted", color = "blue")   + 
  geom_boxplot(outlier.shape = NA, alpha = 0.5 ) +
  theme_bw() + 
  theme(panel.border = element_blank(), panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"), axis.text.x=element_blank(), 
      axis.ticks.x=element_blank())+
  theme(legend.position = "none") +
  labs(y = "sVISa", x = "Station") + ylim(0,0.05)  -> p_sVISa
...
# Calculate the weighted median, 25th percentile, and 75th percentile
weighted_median3 <- weightedMedian(data_1_with_weights$org_charge_density_meq_g_C, w = data_1_with_weights$weight, na.rm = TRUE)
weighted_25th3 <- weightedQuantile(data_1_with_weights$org_charge_density_meq_g_C, w = data_1_with_weights$weight, probs = 0.25, na.rm = TRUE)
weighted_75th3 <- weightedQuantile(data_1_with_weights$org_charge_density_meq_g_C, w = data_1_with_weights$weight, probs = 0.75, na.rm = TRUE)

# Create the boxplot and add the horizontal lines for the weighted median, 25th, and 75th percentiles
data_1 %>%
  mutate( mvm_id = as.factor(mvm_id)) %>%
  ggplot(aes(x = mvm_id, y = org_charge_density_meq_g_C)) +
  geom_rect(aes(xmin = -Inf, xmax = Inf, ymin = weighted_25th3, ymax = weighted_75th3), fill = "lightblue", alpha = 0.2) +
  geom_hline(yintercept = weighted_median3, linetype = "dashed", color = "red") +
  geom_hline(yintercept = weighted_25th3, linetype = "dotted", color = "blue") +
  geom_hline(yintercept = weighted_75th3, linetype = "dotted", color = "blue")   + 
  geom_boxplot(outlier.shape = NA, alpha = 0.5 ) +
  theme_bw() + 
  theme(panel.border = element_blank(), panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"), axis.text.x=element_blank(), 
      axis.ticks.x=element_blank())+
  theme(legend.position = "none") +
  labs(y = "OMCD (meq/g of C)", x = "Station") + ylim(-15,0)-> p_orgd
...
# Calculate the weighted median, 25th percentile, and 75th percentile
weighted_median4 <- weightedMedian(data_1_with_weights$org_charge * 1000, w = data_1_with_weights$weight, na.rm = TRUE)
weighted_25th4 <- weightedQuantile(data_1_with_weights$org_charge * 1000, w = data_1_with_weights$weight, probs = 0.25, na.rm = TRUE)
weighted_75th4 <- weightedQuantile(data_1_with_weights$org_charge * 1000, w = data_1_with_weights$weight, probs = 0.75, na.rm = TRUE)

# Create the boxplot and add the horizontal lines for the weighted median, 25th, and 75th percentiles
data_1 %>%
  mutate(org_charge_mmol_l = org_charge * 1000, mvm_id = as.factor(mvm_id)) %>%
  ggplot(aes(x = mvm_id, y = org_charge_mmol_l)) +
  geom_rect(aes(xmin = -Inf, xmax = Inf, ymin = weighted_25th4, ymax = weighted_75th4), fill = "lightblue", alpha = 0.2) +
  geom_hline(yintercept = weighted_median4, linetype = "dashed", color = "red") +
  geom_hline(yintercept = weighted_25th4, linetype = "dotted", color = "blue") +
  geom_hline(yintercept = weighted_75th4, linetype = "dotted", color = "blue")   + 
  geom_boxplot(outlier.shape = NA, alpha = 0.5 ) +
  theme_bw() + 
  theme(panel.border = element_blank(), panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"), axis.text.x=element_blank(), 
      axis.ticks.x=element_blank())+
  theme(legend.position = "none") +
  labs(y = "Org Charge (mmol/l)", x = "Station")  + ylim(-0.4, 0.05) -> p_orgc
...
```{r}
library(cowplot)
# plot_grid(p_TOC, p_sVISa, p_orgd, p_orgc, labels = "auto", label_fontface = "plain") %>% ggsave(file = "figures/variation_comp.png", width = 35,
#   height = 20,
#   units = "cm",
#   dpi = 300)
```
...
p1 <- plot_variable_distribution(data_1, "TOC_mg_l", "TOC (mg/l)", legend = "No")
p2 <- plot_variable_distribution(data_1, "sVISa", "sVISa" , legend = "No")
p3 <- plot_variable_distribution(data_1 %>% mutate(org_charge_density_meq_g_C = -org_charge_density_meq_g_C), "org_charge_density_meq_g_C", "OMCD (-meq/g C))", legend = "No")
p4 <- plot_variable_distribution(data_1 %>% mutate(org_charge_mmol_l = org_charge *1000), "org_charge_mmol_l", "Org. charge (mmol/l)")

# plot_grid(p1, p2, p3, p4, labels = "auto", label_fontface = "plain", align = "hv") %>% ggsave(file = "figures/variation_kernel.png", width = 35,
#   height = 20,
#   units = "cm",
#   dpi = 300)
...
data_periods <- data_1 %>% 
  mutate(
    period = case_when(
      ...
      sampling_date >= as.Date("1998-01-01") & sampling_date <= as.Date("2012-01-01") ~ "1998-2011",
      sampling_date > as.Date("2012-01-01") ~ "2012-2024",
      TRUE ~ NA_character_  # Handle any NA values if necessary
    ),
   weak_acids = 100*((Sum.of.anions -(-org_charge + 2*SO4_mol + `Cl (mmol/l)`/1000 + `NO3 (mmol/l)`/1000 )) / Sum.of.anions), sampling_date = as.Date(sampling_date),
 strong_acids = 100*((2*SO4_mol + `Cl (mmol/l)`/1000 + `NO3 (mmol/l)`/1000 )/Sum.of.anions),
 organic_acids = (-org_charge / Sum.of.anions) *100 )


```
```{r}
# Find catchments taht havbe data in all three periods 
```
```{r}
#| cache-refresh: true
...
sumtable(data_with_weights %>% mutate(
  ...
  `Sum of Cations (mmol/l)` = Sum.of.cations*1000
    ...
    SO4_mol, charge_diff, Alk_Acid_mol, Sum.of.cations, sample_id, mvm_id
    )) %>% rename(
      `TOC (mg/l)` = TOC_mg_l,
      `Org. Charge Density (-meq/g of C)` = org_charge_density_meq_g_C,
      `EC (mS/M)` = EC_mS_m,
      ...
      `Ionic strength` = Ionic.strength, 
      `Field pH` = pH_ ,
      `NO weak acid contibution (%)` = weak_acids,
      `Strong acid contibution (%)` = strong_acids,
      `Organic acid contribution (%)` = organic_acids
...
variables = c('TOC (mg/l)', 'sVISa', 'Alk Acid (mmol/l)', 'Organic charge (meq/l)', 'Org. Charge Density (-meq/g of C)', "charge difference (%)", 'SO4 (mmol/l)', 'EC (mS/m)', 'Absorbance 420mm', 'Field pH',"Sum of Cations (meq/l)", "Al total (µmol/l)", "Si (mmol/l)", "Cl (mmol/l)" , "F (µmol/l)",                                "NO weak acid contibution (%)",
                                 "Strong acid contibution (%)",
                                  "Organic acid contribution (%)"  )


variables_in = c('TOC (mg/l)', 'sVISa', 'Alk Acid (mmol/l)', 'Organic charge (meq/l)', 'Org. Charge Density (-meq/g of C)', "charge difference (%)", 'SO4 (mmol/l)', 'EC (mS/m)', 'Absorbance 420mm', 'Field pH',"Sum of Cations (meq/l)", "Al total (µmol/l)", "Si (mmol/l)", "Cl (mmol/l)" , "F (µmol/l)" )
...
weighted_summary <- long_data %>%
  filter(variable %in% variables_in) %>%
...
facet_labels <- c(
  ...
  `charge difference (%)` = "f) charge diff. (%)",
...
```{r}
#| label: fig-bar-acid-contribution
#| fig-cap: Bargraphs of the contribution of non-organic weak acids (a), strong acids (b) and organic acids (c) and organic charge density (d) for the three periods, for all stations weighted by the number of samples in each station. Calculated as the percentage of the total negative charge.
#| fig-height: 4
#| fig-width: 8

# Filter the data for the last three variables
last_three_variables <- c("NO weak acid contibution (%)", "Strong acid contibution (%)", "Organic acid contribution (%)", "Org. Charge Density (-meq/g of C)")

# Calculate weighted summary statistics for the last three variables
weighted_summary_last_three <- long_data %>%
  filter(variable %in% last_three_variables) %>%
  group_by(period, variable) %>%
  summarise(
    N = sum(!is.na(value)),
    Median = weightedMedian(value, w = weight, na.rm = TRUE),
...
facet_labels_last_three <- c(
  `NO weak acid contibution (%)`= "b) NO weak acid (%)",
  `Strong acid contibution (%)`= "c) Strong acids (%)",
  `Organic acid contribution (%)` = "d) Organic acids (%)",
  `Org. Charge Density (-meq/g of C)` = "a) Org. Charge Density (-meq/g of C)"
)

# Create the plot with facets for the last three variables
p1 <- ggplot(weighted_summary_last_three %>% filter(variable  == "Org. Charge Density (-meq/g of C)"), aes(x = period, y = Median, fill = period)) +
  geom_bar(stat = "identity") +
  geom_errorbar(aes(ymin = Q25, ymax = Q75), width = 0.2) +
  labs(y = "OMCD (-meq/g of C)", x = "Period") +
  theme_minimal() +
  theme(legend.position = "none",
   strip.text = element_text(hjust = 0)) +
  # facet_wrap(~ variable, scales = "fixed", ncol = 2, labeller = labeller(variable = facet_labels_last_three) ) +
  scale_fill_brewer(palette = "Set2") +
    theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank(), panel.border = element_blank(), panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) + ylim(0,10)

p2 <- ggplot(weighted_summary_last_three %>% filter(variable  == "Organic acid contribution (%)"), aes(x = period, y = Median, fill = period)) +
  geom_bar(stat = "identity") +
  geom_errorbar(aes(ymin = Q25, ymax = Q75), width = 0.2) +
  labs(y = "% contribution", x = "Period") +
  theme_minimal() +
  theme(legend.position = "none",
   strip.text = element_text(hjust = 0)) +
  # facet_wrap(~ variable, scales = "fixed", ncol = 2, labeller = labeller(variable = facet_labels_last_three) ) +
...
p3 <- ggplot(weighted_summary_last_three %>% filter(variable  == "NO weak acid contibution (%)"), aes(x = period, y = Median, fill = period)) +
  geom_bar(stat = "identity") +
  geom_errorbar(aes(ymin = Q25, ymax = Q75), width = 0.2) +
  labs(y = "% contribution", x = "Period") +
  theme_minimal() +
...
p4 <- ggplot(weighted_summary_last_three %>% filter(variable  == "Strong acid contibution (%)"), aes(x = period, y = Median, fill = period)) +
  geom_bar(stat = "identity") +
  geom_errorbar(aes(ymin = Q25, ymax = Q75), width = 0.2) +
  labs(y = "", x = "Period") +
  theme_minimal() +
...
plot_grid(p1, p2, p3, p4, labels = c('   a. OMCD', 'b. Organic acids', 'c. NO weak acids', 'd. Strong acids'), label_fontface = "plain", align = "hv", label_y = 1, label_x = 0) %>%  ggsave(file = "figures/acid_contribution.png", width = 35,
  height = 20,
  units = "cm",
  dpi = 300)
...
#| label: fig-sig-adom_doc
#| out.width: 60%

summary_stats <- data_periods %>% arrange(sampling_date) %>%
  group_by(mvm_id, period) %>%
  summarise(
    median= median(adom_doc, na.rm = TRUE),
    iqr = IQR(adom_doc, na.rm = TRUE),  # Interquartile Range
    n = n()  # Count of observations per group
  ) %>%
  ungroup()
...
# Ensure each group (mvm_id) has more than one period and non-identical adom_doc values across periods
testable_data <- data_periods %>% arrange(sampling_date) %>%
  group_by(mvm_id) %>%
  filter(n_distinct(period) > 1) %>%  # At least two periods
  filter(n_distinct(adom_doc[!is.na(adom_doc)]) > 2) %>%  # At least two unique adom_doc values
  summarise(kruskal_p_value = tryCatch({
    kruskal.test(adom_doc ~ period)$p.value  # Run Kruskal-Wallis only if conditions met
  }, error = function(e) {
...
summary_significance_s <- final_table %>%
...
ggplot(summary_significance_s, aes(x = significant, y = n)) +
  ...
  theme(axis.title.x=element_blank(),
        axis.ticks.x=element_blank(), panel.border = element_blank(), panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) + ylim(0,87)-> p1

p1
```
```{r}
#| cache-refresh: true
#| label: tbl-adom_doc-significance
#| tbl-cap: adom_doc significance using Kruskal Wallis to determine significance and then Dunn's test as a post hoc test to find the significant differences.
...
pairwise_results <- data_periods %>%
  ...
  group_modify(~ {
    tryCatch(
      data.frame(
        dunn_test = list(as.data.frame(dunnTest(adom_doc ~ period, data = .x, method = "bonferroni")$res))
      ),
...
```{r}
# Calculate the percentage changes for each comparison
percentage_changes <- summary_table %>%
  pivot_longer(cols = starts_with("median_"), names_to = "period", values_to = "median") %>%
  mutate(period = str_remove(period, "median_")) %>%
  pivot_wider(names_from = period, values_from = median) %>%
  mutate(
    `1990-1997 - 1998-2011` = (`1998-2011` - `1990-1997`) / `1990-1997` * 100,
    `1990-1997 - 2012-2024` = (`2012-2024` - `1990-1997`) / `1990-1997` * 100,
    `1998-2011 - 2012-2024` = (`2012-2024` - `1998-2011`) / `1998-2011` * 100
  ) %>%
  select(mvm_id, `1990-1997 - 1998-2011`, `1990-1997 - 2012-2024`, `1998-2011 - 2012-2024`) %>%
  pivot_longer(cols = -mvm_id, names_to = "dunn_test.Comparison", values_to = "MedianChange")

  # Merge the percentage changes with the pairwise results
pairwise_results <- pairwise_results %>%
  left_join(percentage_changes, by = c("mvm_id", "dunn_test.Comparison"))
```

```{r}
#| fig.cap: adom_doc changes between periods. Only stations for which Kruskall-Wallis test found a significant difference between periods are included. In the tile is indicated how many station out of all the stations that had significant results between the periods had data for a comparison during the specified periods.
#| label: fig-bar-adom_doc
#| fig-width: 8
#| fig-height: 8

library(forcats)
pairwise_results %>% mutate(significance = case_when(
    dunn_test.P.adj >= 0.05~ "not sig", 
    dunn_test.P.adj < 0.05 & MedianChange > 0 & MedianChange < 5 ~ "sig increase < 5%",
    dunn_test.P.adj < 0.05 & MedianChange > 5 ~ "sig increase > 5%",
    dunn_test.P.adj < 0.05 & MedianChange < 0 & MedianChange > -5 ~ "sig decrease < 5%",
    dunn_test.P.adj < 0.05 & MedianChange < -5 ~ "sig decrease > 5%")) %>%
  ...
  mutate(dunn_test.Comparison = fct_relevel(dunn_test.Comparison, "1990-1997\n1998-2011", "1998-2011\n2012-2024", "1990-1997\n2012-2024")) -> pairwise_results_1


ggplot(pairwise_results_1, aes(x = as.factor(mvm_id), fill = significance)) +
  geom_bar(position = "fill") +
  facet_wrap(~ dunn_test.Comparison) +
  labs(x = "Station (mvm_id)", y = "Proportion", fill = "Significance") +
  theme_minimal() +
  scale_fill_manual(values = c("sig increase < 5%" = "#E69F00", "sig decrease < 5%" = "#56B4E9", "not sig" = "#F0E442", "no data" = 'white',"sig increase > 5%" = "#D55E00", "sig decrease > 5%" = "#0072B2" )) +
  coord_flip() +
  ylab("Dunn's test")+
  xlab("Stations")+
...
```{r}
#| tab-cap: Information on how many stations had data for each period for adom_doc.
#| label: table-significance-adom_doc
library(dplyr)

# Calculate the total number of stations for each period
total_stations_per_period <- pairwise_results_1 %>%
...
stations_with_data <- pairwise_results_1 %>%
...
```{r}
#| fig.cap: adom_doc of all stations not weighted.
#| fig.subcap:
#|      - "Median"
#|      - "IQR"
#| layout-ncol: 2
#| label: fig-box-adom_doc
#| 
ggplot(summary_stats, aes(x = period, y = median, fill = period)) +
  ...
  labs(x = "Period", y = "Median adom_doc") +
  theme_minimal() +
  theme(legend.position = "none")
...
# Create a summary table that counts the number of mvm_ids with significant and non-significant differences:
summary_significance_TOC <- final_table %>%
  mutate(significant = case_when (kruskal_p_value < 0.05 ~ "Significant", kruskal_p_value >= 0.05 ~"Not Significant", 
  TRUE ~ "No Data")) %>% as_tibble() %>%
  dplyr::count(significant)
...
ggplot(summary_significance_TOC, aes(x = significant, y = n)) +
  geom_bar(stat = "identity", fill  = "grey", colour = "grey") +
  geom_text(aes(label = n), vjust = -0.5) +
  theme_bw() +
  labs(title = "",
  ...
  theme(axis.title.x=element_blank(),
        axis.ticks.x=element_blank(), panel.border = element_blank(), panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))  + ylim(0,87) -> p2
p2
```
```{r}
#| cache-refresh: true
...
```{r}
# Calculate the median changes for each comparison
# Calculate the percentage changes for each comparison
percentage_changes <- summary_table %>%
  pivot_longer(cols = starts_with("median_"), names_to = "period", values_to = "median") %>%
  mutate(period = str_remove(period, "median_")) %>%
...
if (re.read == 're read'){
data_12 %>%  
  filter(!is.na(org_charge_density_meq_g_C)) %>% 
  mutate(sampling_date = as.Date(sampling_date)) %>% 
  group_by(mvm_id) %>%
  filter({
    # Count samples per year
    yearly_counts <- as.vector(table(year(sampling_date)))
    # Find consecutive years with at least 6 samples
    consecutive_years <- rle(yearly_counts >= 6)
    # Identify the longest series of consecutive years with 6+ samples
    max_consecutive <- ifelse(length(consecutive_years$lengths[consecutive_years$values]) > 0, 
                              max(consecutive_years$lengths[consecutive_years$values]), 0)
    # Keep only if the longest series is at least 6 years
    max_consecutive >= 6
  }) %>%
  ungroup()   %>%
  select(mvm_id, org_charge_density_meq_g_C, sampling_date) -> data_filtered
     
data_filtered %>% 
  mutate(SiteID = as.factor(mvm_id)) %>% 
  pivot_longer(cols = c("org_charge_density_meq_g_C")) %>% 
  screeningmodeling(values = value,
                    datevar = sampling_date, 
                    link = "identity", 
                    conf.type = "conf",
                    ...
                    autocor = TRUE,
                    mvm_id) ->
  trendplotdata_selected_OMCD

trendplotdata_selected_OMCD %>% 
  left_join(coordinates) %>% 
  saveRDS(., file = "../data/trendplotdata_selected_omcd.rds")
}  else if (re.read == 're load') {
  trendplotdata_selected_OMCD <- tryCatch({
    readRDS(file = "../data/trendplotdata_selected_omcd.rds")
  }, error = function(e) {
    message("An error occurred while reading the RDS file: ", e$message)
    NULL # Return NULL or an alternative value to handle the error gracefully
  })
...
```{r}
#| include: false
# Trend plot for org_charge_density_meq_g_C

if (re.read != 're fig' & exists("trendplotdata_selected_OMCD")){
  fig <- trendplotdata_selected_OMCD %>%plot_proportions()+ ggtitle("OMCD") + 
        theme(text=element_text(size=5), #change font size of all text
        axis.text=element_text(size=5), #change font size of axis text
        axis.title=element_text(size=6), #change font size of axis titles
        plot.title=element_text(size=6.5), #change font size of plot title
        legend.text=element_text(size=6), #change font size of legend text
        legend.title=element_text(size=5)) 
  fig %>% ggsave(file = "figures/omcd_proportion_selected_filtered.png", width = 10,
  height = 6,
  units = "cm",
  dpi = 300) 
}  
...
# Create a summary table that counts the number of mvm_ids with significant and non-significant differences:
summary_significance_ocd <- final_table %>%
  mutate(significant = case_when (kruskal_p_value < 0.05 ~ "Significant", kruskal_p_value >= 0.05 ~"Not Significant", 
  TRUE ~ "No Data")) %>% as_tibble() %>%
  dplyr::count(significant)
...
ggplot(summary_significance_ocd, aes(x = significant, y = n)) +
  geom_bar(stat = "identity", fill  = "grey", colour = "grey") +
  geom_text(aes(label = n), vjust = -0.5) +
  theme_bw() +
  labs(title = "",
  ...
  theme(axis.title.x=element_blank(),
        axis.ticks.x=element_blank(), panel.border = element_blank(), panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))  + ylim(0,87) -> p3
p3
```
```{r}
#| cache-refresh: true
...
percentage_changes <- summary_table %>%
  ...
  pivot_wider(names_from = period, values_from = median) %>%
  mutate(
    `1990-1997 - 1998-2011` = (`1998-2011` - `1990-1997`) / abs(`1990-1997`) * 100,
    `1990-1997 - 2012-2024` = (`2012-2024` - `1990-1997`) / abs(`1990-1997`) * 100,
    `1998-2011 - 2012-2024` = (`2012-2024` - `1998-2011`) / abs(`1998-2011`) * 100
  ) %>%
  select(mvm_id, `1990-1997 - 1998-2011`, `1990-1997 - 2012-2024`, `1998-2011 - 2012-2024`) %>%
  pivot_longer(cols = -mvm_id, names_to = "dunn_test.Comparison", values_to = "MedianChange")
...
```{r}
#| fig-cap: Organic charge density changes between periods. Only stations for which Kruskall-Wallis test found a significant difference between periods are included. In the tile is indicated how many station out of all the stations that had significant results between the periods had data for a comparison during the specified periods. Color indicates also if the change in median organic charge density is greater than 5%. 
...
ggplot(summary_stats, aes(x = period, y = median, fill = period)) +
  ...
  #   label = paste0("sig n: ", sig_n, "\ninsig n: ", insig_n)
  # ), color = "black", size = 4, position = position_dodge(width = 0.9)) +
  labs(x = "Period", y = "Median OMCD (meq/g of C)") +
  theme_minimal() +
  theme(legend.position = "none")+
  theme(axis.title.x=element_blank(),
        axis.ticks.x=element_blank(), panel.border = element_blank(), panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) -> p_med
...
ggplot(summary_stats, aes(x = period, y = iqr, fill = period)) +
  ...
  #   label = paste0("sig n: ", sig_n, "\ninsig n: ", insig_n)
  # ), color = "black", size = 4, position = position_dodge(width = 0.9)) +
  labs(x = "Period", y = "IQR OMCD (meq/g of C)") +
  theme_minimal() +
  theme(legend.position = "none") +
  theme(axis.title.x=element_blank(),
        axis.ticks.x=element_blank(), panel.border = element_blank(), panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) -> p_iqr

plot_grid(p_med, p_iqr,  labels = c('a', 'b'), ncol = 2, label_fontface = "plain", align = "hv", label_y = 1, label_x = 0) %>%  ggsave(file = "figures/med_iqr_OMCD.png", width = 30,
  height = 10,
  units = "cm",
  dpi = 300)
```
...
<!-- Acids in a water chemistry sample can be split into weak acids and strong acids. The weak acids can be further divided into organic acids and other weak acids. Here we look at how the contribution of these non-organic other weak acids has changed over time for each station. How it has changed for all stations combined can be seen in @fig-bar-acid-contribution. -->

The weak acid contribution is calculated as: 

$$
{\substack{\text{charge contribution of} \\ \text{ non-organic weak anions}}} =  100 * \frac{\text{Sum of anions} - (\text{Organic anions} + \text{Sum of strong anions})}{\text{Sum of anions}}
$$
...
|**organic acids**| DOC | modelled|
 : The components of the sum of anions for each water chemistry sample, its constituents and whether it is modelled or measured. {#tbl-anions}
...
#| fig.cap: "Kruskal Wallis with confidence interval of 95% to find number of stations with significant differences bettwen the periods. "
#| label: fig-sig-weak_acids
#| out.width: 60%

summary_stats <- data_periods  %>%
  arrange(sampling_date) %>%
  group_by(mvm_id, period) %>%
  summarise(
    median= median(weak_acids, na.rm = TRUE),
    iqr = IQR(weak_acids, na.rm = TRUE),  # Interquartile Range
    n = n()  # Count of observations per group
  ) %>%
  ungroup()
...
# Step 4: Filter data for Kruskal-Wallis testing
# Ensure each group (mvm_id) has more than one period and non-identical weak_acids values across periods
testable_data <- data_periods %>%
  arrange(sampling_date) %>%
  group_by(mvm_id) %>%
  filter(n_distinct(period) > 1) %>%  # At least two periods
  filter(n_distinct(weak_acids[!is.na(weak_acids)]) > 2) %>%  # At least two unique weak_acids values
  summarise(kruskal_p_value = tryCatch({
    kruskal.test(weak_acids ~ period)$p.value  # Run Kruskal-Wallis only if conditions met
  }, error = function(e) {
    message("Error in Kruskal-Wallis test for mvm_id ", unique(mvm_id), ": ", e$message)
    NA
  }))
...
# Create a summary table that counts the number of mvm_ids with significant and non-significant differences:
summary_significance_wa <- final_table %>%
  mutate(significant = case_when (kruskal_p_value < 0.05 ~ "Significant", kruskal_p_value >= 0.05 ~"Not Significant", 
  TRUE ~ "No Data")) %>% as_tibble() %>%
  dplyr::count(significant)
...
ggplot(summary_significance_wa, aes(x = significant, y = n)) +
  geom_bar(stat = "identity", fill  = "grey", colour = "grey") +
  geom_text(aes(label = n), vjust = -0.5) +
  theme_bw() +
  labs(title = "",
  ...
  theme(axis.title.x=element_blank(),
        axis.ticks.x=element_blank(), panel.border = element_blank(), panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))  + ylim(0,87) -> p4
p4
```
...
```{r}
#| cache-refresh: true
#| label: tbl-weak_acids-significance
#| tbl-cap: Contribution of weak acids significance using Kruskal Wallis to determine significance and then Dunn's test as a post hoc test to find the significant differences.
...
pairwise_results <- data_periods %>%
  ...
  group_modify(~ {
    tryCatch(
      data.frame(
        dunn_test = list(as.data.frame(dunnTest(weak_acids ~ period, data = .x, method = "bonferroni")$res))
      ),
      error = function(e) data.frame(dunn_test_summary = list(NULL))  # Handle errors
    )
  }) %>%
...
```{r}
#| fig.cap: Contribution of non-organic weak acids to charge changes between periods. Only stations for which Kruskall-Wallis test found a significant difference between periods are included. In the tile is indicated how many station out of all the stations that had significant results between the periods had data for a comparison during the specified periods.
#| label: fig-bar-weak_acids
#| fig-width: 8
#| fig-height: 8
...
```{r}
#| tab-cap: Information on how many stations had data for each period for the contribution of weak acids to charge.
#| label: table-significance-weak_acids
library(dplyr)
...
```{r}
#| fig.cap: Contribution of strong acids to charge of all stations not weighted.
#| fig.subcap:
#|      - "Median"
#|      - "IQR"
#| layout-ncol: 2
#| label: fig-box-strong_acids
ggplot(summary_stats, aes(x = period, y = median, fill = period)) +
  geom_boxplot() +
  # geom_text(aes(
  #   x = period,
...
plot_grid(p_med, p_iqr,  labels = c('a', 'b'), ncol = 2, label_fontface = "plain", align = "hv", label_y = 1, label_x = 0) %>%  ggsave(file = "figures/med_iqr_WA.png", width = 30,
  height = 10,
  units = "cm",
  dpi = 300)
```
...
```{r}
#| fig.cap: Contribution of weak acids to charge of all stations not weighted.
#| fig.subcap:
#|      - "Median"
#|      - "IQR"
#| layout-ncol: 2
#| label: fig-box-weak_acids
#| 
ggplot(summary_stats, aes(x = period, y = median, fill = period)) +
  geom_boxplot() +
  # geom_text(aes(
  ...
  #   label = paste0("sig n: ", sig_n, "\ninsig n: ", insig_n)
  # ), color = "black", size = 4, position = position_dodge(width = 0.9)) +
  labs(x = "Period", y = "Median contribution weak_acids") +
  theme_minimal() +
  theme(legend.position = "none")
...
ggplot(summary_stats, aes(x = period, y = iqr, fill = period)) +
  ...
  #   label = paste0("sig n: ", sig_n, "\ninsig n: ", insig_n)
  # ), color = "black", size = 4, position = position_dodge(width = 0.9)) +
  labs(x = "Period", y = "IQR weak_acids") +
  theme_minimal() +
  theme(legend.position = "none")
```

```{r}
plot_grid(p2, p1, p3, p4, labels = c('    a. TOC', '     b. sVISa', '    c. OMCD', 'd. NO weak acids'), label_fontface = "plain", align = "hv", label_y = 1, label_x = 0) %>%  ggsave(file = "figures/significants.png", width = 35,
  height = 20,
  units = "cm",
  dpi = 300)

plot_grid(p2, p1, p3,  labels = c('a. TOC', 'b. sVISa', 'c. OMCD'), ncol = 3, label_fontface = "plain", align = "hv", label_y = 1, label_x = 0) %>%  ggsave(file = "figures/significants_3.png", width = 30,
  height = 10,
  units = "cm",
  dpi = 300)
```
...
data_12 %>%  
  ...
  ungroup()   %>%
  select(mvm_id, `Al total (µmol/l)`, sampling_date) -> data_filtered
     
data_filtered %>% 
  mutate(SiteID = as.factor(mvm_id)) %>% 
  pivot_longer(cols = c("Al total (µmol/l)")) %>% 
  screeningmodeling(values = value,
                    datevar = sampling_date, 
                    link = "identity", 
                    conf.type = "conf",
                    ...
                    autocor = TRUE,
                    mvm_id) ->
  trendplotdata_selected_al



trendplotdata_selected_al %>% 
  left_join(coordinates) -> trendplotdata_selected_al
  
 trendplotdata_selected_al %>% 
  saveRDS(., file = "../data/trendplotdata_selected_filtered_al_tot.rds")
}  
...
if (re.read != 're fig' & exists("trendplotdata_selected_al")){
  fig <- trendplotdata_selected_al %>%plot_proportions()+ ggtitle("OMCD") + 
        theme(text=element_text(size=5), #change font size of all text
        axis.text=element_text(size=5), #change font size of axis text
        axis.title=element_text(size=6), #change font size of axis titles
        plot.title=element_text(size=6.5), #change font size of plot title
        ...
        legend.title=element_text(size=5))
  fig       
  fig %>% ggsave(file = "figures/al_proportion_selected_filtered.png", width = 10,
  height = 6,
  units = "cm",
  dpi = 300) 
}  
...
```{r}
plot_loading_ggplot <- function(opls_model, response_label = "TOC", vip.threshold = 1.0, text_size = 10, title = "Hella Sverige") {
  library(ggplot2)
  library(dplyr)
...
library(tibble)
source("../src/sourcecode.R")
plot_loading_ggplot(opls_model = opls.model, response_label = "OMCD", vip.threshold = 1.0, text_size = 10)
...
plot_loading_ggplot(opls.model.sVISa, response_label = "sVISa", vip.threshold = 1.0, text_size = 10)
```
...
```{r}
#| cache-refresh: false
plot_loading_ggplot(opls.model.TOC, response_label = "TOC", vip.threshold = 1.0, text_size = 10)
```
...
```{r VIP table}
#| cache-refresh: false
#| tbl-cap: Table of OPLS results, showing the variation explained by the model (R2) the model efficiency based on seven fold validatino (Q2) and the explanatory variables with a VIP score > 0.9. The first row is for median organic charge density, the second row for median TOC concentrations, and the third column for sVISa.
#| label: table-PLS-comp
